...
Único cambio de API: get_channel_config_by_telegram_id es la función
principal de lookup; ya no se usa channel_name como clave.
Añade columnas enable_reverse, magic_reverse, execution_type_rev al schema.
"""
import os
import sqlite3
import time
...
def use_backtest_db():
    global DB_PATH; DB_PATH = DB_PATH_BT

def use_real_db():
    global DB_PATH; DB_PATH = DB_PATH_REAL
...
CREATE TABLE IF NOT EXISTS raw_messages (
    id INTEGER PRIMARY KEY, channel_id TEXT, channel_name TEXT,
    message_id INTEGER, date INTEGER, text TEXT, raw_json TEXT
);
CREATE TABLE IF NOT EXISTS signals (
    id INTEGER PRIMARY KEY, raw_message_id INTEGER, channel_name TEXT,
    magic INTEGER, symbol TEXT, action TEXT, direction TEXT,
    risk_pct REAL, volume_hint REAL, sl REAL, tp REAL,
    created_at INTEGER, status TEXT, tg_message_id INTEGER,
    tg_reply_to_id INTEGER, tp_label TEXT, entry_min REAL, entry_max REAL,
    tp1 REAL, tp2 REAL, tp3 REAL, tp4 REAL, tp5 REAL,
    tp6 REAL, tp7 REAL, tp8 REAL, tp9 REAL, tp10 REAL,
    tp_stage INTEGER DEFAULT 0, execution_type TEXT DEFAULT 'PENDING',
    executed_by TEXT, executed_at INTEGER, error_msg TEXT,
    exported_mt4 INTEGER DEFAULT 0
);
CREATE TABLE IF NOT EXISTS executions (
    id INTEGER PRIMARY KEY, signal_id INTEGER, ticket INTEGER,
    symbol TEXT, volume REAL, price_open REAL, price_close REAL,
    sl REAL, tp REAL, close_reason TEXT, profit REAL,
    time_open INTEGER, time_close INTEGER
);
CREATE TABLE IF NOT EXISTS channel_config (
    id INTEGER PRIMARY KEY, channel_name TEXT UNIQUE NOT NULL,
    telegram_id INTEGER UNIQUE NOT NULL, magic INTEGER NOT NULL,
    risk_pct REAL DEFAULT 2.0, execution_type TEXT DEFAULT 'PENDING',
    enabled INTEGER DEFAULT 1, enable_reverse INTEGER DEFAULT 0,
    magic_reverse INTEGER, execution_type_rev TEXT,
    created_at INTEGER, updated_at INTEGER, partial_tp_config TEXT
);
CREATE TABLE IF NOT EXISTS channel_brokers (
    id INTEGER PRIMARY KEY, channel_id INTEGER NOT NULL,
    broker_name TEXT NOT NULL, broker_magic INTEGER,
...
def insert_raw_message(data: Dict[str, Any]) -> int:
    ...
    cur.execute(
        ...
        " VALUES (:channel_id,:channel_name,:message_id,:date,:text,:raw_json)", data)
    conn.commit(); rid = cur.lastrowid; conn.close(); return rid

def insert_signal(data: Dict[str, Any]) -> int:
    conn = get_connection(); data = dict(data)
    for k in ("risk_pct","volume_hint","entry_min","entry_max","magic","tp_label",
              "error_msg","tp","sl","tp1","tp2","tp3","tp4","tp5",
              "tp6","tp7","tp8","tp9","tp10"):
        data.setdefault(k, None)
    data.setdefault("status","PENDING"); data.setdefault("tp_stage",0)
    data.setdefault("action",None); data.setdefault("direction",None)
    data.setdefault("symbol",None); data.setdefault("execution_type","MARKET")
    ...
    cur.execute("""
        INSERT INTO signals (
            ...
            tp1,tp2,tp3,tp4,tp5,tp6,tp7,tp8,tp9,tp10, tp_stage, execution_type
        ) VALUES (
            :raw_message_id,:channel_name,:magic,:symbol,:action,
            :direction,:risk_pct,:volume_hint,:sl,:tp,:created_at,:status,
            :tg_message_id,:tg_reply_to_id,:tp_label,:entry_min,:entry_max,
...
def insert_sl_move(symbol, channel_name, new_sl, tg_reply_to_id) -> int:
    conn = get_connection(); cur = conn.cursor()
    cur.execute(
        "INSERT INTO signals (action,symbol,channel_name,sl,tg_reply_to_id,status)"
        " VALUES ('SL_MOVE',:symbol,:channel_name,:sl,:tg_reply_to_id,'PENDING')",
        {"symbol":symbol,"channel_name":channel_name,"sl":new_sl,"tg_reply_to_id":tg_reply_to_id})
    conn.commit(); rid = cur.lastrowid; conn.close(); return rid
